...
#define SIZ_TABLE 6320 // size of hash table = number of words in library
...
t_node* getnode()
{
  t_node* return_node = NULL;
  return_node = (t_node*)malloc(sizeof(t_node));


  if(return_node)
    {
      return_node-> isend=0;
      memset(return_node->CHILDREN, 0, 26*sizeof(char));
    }
...
void trie_insert(t_node* root,char* str)
{
  int i;
...
int search(t_node* root, char* key)
{
  if (root == NULL)
    return 0;
  int i;
...
int main()
{
  ...
  FILE* file_ptr;
  char buf[SIZ_STR];
  file_ptr = fopen("rand_file_small.txt", "r");

  t_node** hash_ptr;
  t_node* hash_table[SIZ_TABLE];
  memset(hash_table, 0, SIZ_TABLE*sizeof(t_node*));
  unsigned int i;
  hash_ptr = hash_table;
